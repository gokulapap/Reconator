...
- Pinned isolated implementations for multi-source passive subdomains (Subfinder), wildcard-aware structured DNS validation (DNSX), historical URLs (URLFinder), HTTP/TLS/ASN/CDN/technology enrichment (HTTPX), same-host crawling (Katana), TCP-connect port discovery (Naabu), AST-aware JavaScript analysis (JSLuice), bounded name mutations (AlterX), and local CDN/cloud/WAF range classification (CDNCheck).
- Dynamic capability consumers, plugin discovery, and persisted parallel-source,
  preferred-then-fallback, and sequential-enrichment execution policies.
...
6. Validation implementations: batched DNS brute-force validation, virtual-host comparison, bounded content discovery, service fingerprinting, and response-similarity suppression with per-program safety profiles.
...
9. Maximum-yield DNS: a checksum-pinned maintained wordlist, PureDNS/MassDNS-style batched validation with resolver health management, bounded recursive zone discovery, corpus-driven mutations, and chunked checkpointed ingestion for source results above one task's in-memory budget.

## Platform improvements

- Distributed global/provider rate-limit buckets rather than database-visible per-target start spacing.
- Conditional/consensus capability policies driven by normalized result quality, source
  health, and bounded scan budgets.
